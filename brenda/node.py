...
def start_s3_push_process(opts, args, conf, outdir, project_name):
    p = Multiprocess(target=s3_push_process, args=(opts, args, conf, outdir, project_name))
    p.start()
    return p

def s3_push_process(opts, args, conf, outdir, project_name):
    def do_s3_push():
        bucktup = aws.get_s3_output_bucket(conf, project_name)
        ...
        try:
            ...
            while True:
                ...
                if task.msg is not None:
                    ...
                    try:
                        task_obj = json.loads(task.msg.get_body())
                    except ValueError:
                        print "bad input task"
                        continue

                    project_name = task_obj['project-name']

                    work_dir = aws.get_work_dir(conf)
                    proj_dir = os.path.join(work_dir, "brenda-project.tmp", project_name)
                    if not os.path.exists(proj_dir):
                        utils.makedirs(proj_dir)
                        get_s3_project(conf, blender_project + '/' + project_name, proj_dir)
                ...
                if local.task_active:
                    local.task_active.proc = start_s3_push_process(opts, args, conf, local.task_active.outdir, project_name)
